...
selectInput(inputId = "day", 
            label = h4("Day of the Week"), 
            choices = c("Mon","Tues", "Wedn", "Thur", "Fri" , "Sat", "Sun"), 
            selected = c("Mon"))

checkboxGroupInput("d1.substance", label = h3("Driver 1 Substances"), 
    choices = list("Alcohol",
                   "Drugs",
                   "No Apparent Influence"),
    selected = c("No Apparent Influence"))

checkboxGroupInput("d2.substance", label = h3("Driver 2 Substances"), 
    choices = list("Alcohol",
                   "Drugs",
                   "No Apparent Influence"),
    selected = c("No Apparent Influence"))
...
### Violations and Day of Week
```{r}
# LEAFLET MAPPING
renderLeaflet({

d2 <- dat %>%
    filter(dat$day == input$day,
      d1.substance %in% input$d1.substance,
...
```{r}
checkboxGroupInput("timeofday", label = h3("Select Time of Day"), 
...
checkboxGroupInput("weezy", label = h3("Select Weather Condition"), 
    choices = unique(dat$Weather),  # Using the unique() to get all values 
    selected = c("Clear"))
...
renderLeaflet({
  ...
  d2 <- dat %>%
    filter( time.of.day %in% input$timeofday, # filtering by time of day 
            Weather %in% input$weezy)         # filtering by weather 
...
This dashboard has two new tabs that showcase the following crashes during different weather and time of day conditions (made in class), and the prevalence of substance abuse during different days of the week.

### Dashboard Author
Anna Hall is a Political Science and Economics student at Georgia State University. She will be graduating in Summer 2023 and has a history of working with data on political campaigns.
